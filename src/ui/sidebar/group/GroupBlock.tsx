...
import { clientStates, createGroup, leaveUserFromGroup } from "../../../client/Client";
import { Button } from "../../../components/ui/button";
import { Popover, PopoverContent, PopoverTrigger } from "../../../components/ui/popover";
import { cn } from "../../../lib/utils";
import SelectFriendDialog from "../friends/dialogs/SelectFriendDialog";
  ...
  return (
    <Popover open={(isParticipant && !isMeInvited && isMe && open) || (open && !isParticipant)}>
      <PopoverTrigger onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        ...
        >
          {state === 3 ? <Clock className="absolute right-[1px] top-[1px] z-20 opacity-[.2] w-[16px] h-[16px]" /> : null}
          {state !== 3 && isLeader ? <Crown className="absolute left-[1px] top-[1px] w-[16px] h-[16px] text-yellow-500" /> : null}
          <img src={user.avatar_url} className={cn("pointer-events-none", state === 3 ? "grayscale" : undefined)} />
        </div>
      </PopoverTrigger>
...
export const GroupBlock = observer(() => {
  ...
  const handleFriendSelected = async (friend: Friend) => {
    ...
    await createGroup(friend.user.id, friend.user.username);
  };
  ...
  return (
    <Fragment>
      ...
      <div className="flex flex-row gap-1 p-1 bg-gray-600">
        ...
        {emptyLength > 0
          ? new Array(emptyLength).fill(() => 1).map((_, index) => <EmptyUserItem key={index} openAddUserDialog={() => setAddUserOpen(true)} />)
          : null}
      </div>
    </Fragment>
  );
